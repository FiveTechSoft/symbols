...
<!-- METRICS:BEGIN -->
Measured on commit `15fa1a4` on 2026-09-23 (linux, build) with `python3 tools/metrics.py`.

| Metric | Value | How it is measured |
|---|---|---|
| CTest suite | 86/97 pass, 1 fail, 10 skipped | full `ctest`; skipped = optional data missing |
| Unit asserts | 785 pass, 0 fail | sum of `TEST RESULTS` over all tests |
...
| Server latency | p50 1.7 ms, p95 2.8 ms | same set, local |
...
| Engineering task bank (56 tasks, 8 categories) | `symbols-agent`: 0/56 pass (0%), 0 wrong edits, 56 untouched; by category: build_ci 0/7, compiler_repair 0/7, debug 0/7, docs 0/7, multi_file 0/7, refactor 0/7, shell 0/7, test_authoring 0/7 | `tools/bank_harness.py` (before/ + task.md + check.py; golden after/ only in `--self-test`, self-test 56/56) |
| Wikidata QA evaluation (`test_eval_*`) | not measured | `wiki_model.bin` missing (not bundled) |
| CI per platform | apply: success, ci / asan-msvc: in_progress, ci / build-test-linux: success, ci / build-test-msvc: success | [run](https://github.com/FiveTechSoft/symbols/actions/runs/35838048500); failing tests are listed in each job log |
