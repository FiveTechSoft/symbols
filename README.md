...
<!-- METRICS:BEGIN -->
Measured on commit `6e9417d` on 2026-09-23 (linux, build) with `python3 tools/metrics.py`.
...
| CI per platform | asan-msvc: in_progress, build-test-linux: success, build-test-msvc: success | [run](https://github.com/FiveTechSoft/symbols/actions/runs/35846898895); failing tests are listed in each job log |
| Hand-written rules (declared) | 19 rules; tables: `tools.tsv` 16 rows, `fixtures.tsv` 5 rows, `english-spanish.txt` 357 rows | `tools/metrics/declared_rules.tsv` (the script checks each one is still in the code) |
<!-- METRICS:END -->
