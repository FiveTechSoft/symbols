...
<!-- METRICS:BEGIN -->
Measured on commit `cca91ab` on 2026-09-23 (linux, build) with `python3 tools/metrics.py`.
...
| Server latency | p50 1.5 ms, p95 2.6 ms | same set, local |
...
| Engineering task bank (56 tasks, 8 categories) | `symbols-agent`: 24/56 pass (43%; dev 19/32, retired held-out 5/24 (seen since fa230f4; blind: none yet)), 0 wrong edits, 32 untouched; by category: build_ci 0/7, compiler_repair 5/7, debug 3/7, docs 3/7, multi_file 5/7, refactor 3/7, shell 0/7, test_authoring 5/7 | `tools/bank_harness.py` (before/ + task.md + check.py; golden after/ only in `--self-test`, self-test 56/56) |
