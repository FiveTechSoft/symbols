...
<!-- METRICS:BEGIN -->
Measured on commit `465bd02` on 2026-09-23 (linux, build) with `python3 tools/metrics.py`.
...
| Server latency | p50 1.4 ms, p95 2.0 ms | same set, local |
...
| CI per platform | apply: success, ci / asan-msvc: in_progress, ci / build-test-linux: success, ci / build-test-msvc: in_progress | [run](https://github.com/FiveTechSoft/symbols/actions/runs/35851356721); failing tests are listed in each job log |
| Hand-written rules (declared) | 27 rules; tables: `tools.tsv` 16 rows, `fixtures.tsv` 5 rows, `english-spanish.txt` 357 rows | `tools/metrics/declared_rules.tsv` (the script checks each one is still in the code) |
<!-- METRICS:END -->
