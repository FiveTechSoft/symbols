...
<!-- METRICS:BEGIN -->
Measured on commit `ba61e56` on 2026-09-23 (linux, build) with `python3 tools/metrics.py`.

| Metric | Value | How it is measured |
|---|---|---|
| CTest suite | 88/98 pass, 0 fail, 10 skipped | full `ctest`; skipped = optional data missing |
| Unit asserts | 814 pass, 0 fail | sum of `TEST RESULTS` over all tests |
...
| Engineering task bank (56 tasks, 8 categories) | `symbols-agent`: 12/56 pass (21%; dev 7/32, held-out 5/24), 0 wrong edits, 44 untouched; by category: build_ci 1/7, compiler_repair 4/7, debug 2/7, docs 0/7, multi_file 4/7, refactor 0/7, shell 0/7, test_authoring 1/7 | `tools/bank_harness.py` (before/ + task.md + check.py; golden after/ only in `--self-test`, self-test 56/56) |
| Wikidata QA evaluation (`test_eval_*`) | not measured | `wiki_model.bin` missing (not bundled) |
| CI per platform | apply: success, ci / asan-msvc: in_progress, ci / build-test-linux: success, ci / build-test-msvc: success | [run](https://github.com/FiveTechSoft/symbols/actions/runs/35842673370); failing tests are listed in each job log |
| Hand-written rules (declared) | 15 rules; tables: `tools.tsv` 16 rows, `fixtures.tsv` 5 rows, `english-spanish.txt` 357 rows | `tools/metrics/declared_rules.tsv` (the script checks each one is still in the code) |
<!-- METRICS:END -->
