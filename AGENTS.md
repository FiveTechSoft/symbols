...
Los resultados de pruebas y conteos citados abajo son registros históricos de las ejecuciones fechadas, no evidencia de la matriz del SHA actual. La implementación está acotada por [README §6-7](README.md#6-persistent-memory-implemented-behavior-and-limits). Los límites de los pilotos recientes se detallan en `docs/abstain-and-ask.md` y `docs/ast-inspect.md`.
...
- OpenCode opera con Symbolic LLM como su único proveedor LLM sin MCP ni llamadas a APIs de terceros. El endpoint HTTP `symbols_server` (`/v1/chat/completions`) implementa el protocolo wire de OpenAI Tool Calling: deserializa esquemas `tools`, ingesta retornos `role: "tool"` en bucle ReAct, formula planes STRIPS acotados para los casos implementados y emite `tool_calls` con `finish_reason: "tool_calls"` en los flujos de prueba hasta cerrar con reporte Markdown (`finish_reason: "stop"`), manteniendo en paralelo el canal de consultas factuales directas. Validado por suite CTest 31/31 (`test_server_tool_calling.c`) y test de integración por socket HTTP real de 8 turnos (`tools/test_opencode_agentic_server.py`), 2026-09-20.

## Motor Abductivo de Diagnóstico y Linters (Fase 7)

- El motor abductivo (`src/agent_diagnose.c`, `include/agent_diagnose.h`) parsea salidas terminales de compiladores (GCC, Clang y MSVC) extrayendo posición de error, clasificación taxonómica formal (`DIAG_ERR_UNDECLARED_SYMBOL`, `MISSING_MEMBER`, `ARITY_MISMATCH`, `TYPE_MISMATCH`, `MISSING_HEADER`, `SYNTAX`, `REDEFINITION`), sugerencias "did you mean" y enlace abductivo directo con el Grafo de Conocimiento del Código (`CodeGraphGetFunctionFile`). Activa dinámicamente predicados STRIPS (`PRED_ERROR_DIAGNOSED`) disparando replanificación curativa en los casos cubiertos por las pruebas. Validado por suite CTest 39/39 (`tests/test_agent_diagnose.c`) y 0 regresiones en suites agénticas, 2026-09-20.

## Grafo de Conocimiento del Código Políglota (Fase 8)

- El motor de grafo de código (`src/code_graph.c`, `include/code_graph.h`) se extiende con extracción léxica políglota para Python (`.py`, `.pyw`) y TypeScript/JavaScript (`.ts`, `.tsx`, `.js`, `.jsx`, `.mjs`, `.cjs`) sin dependencias externas. Extrae clases, herencia (`inherits_from`/`inherited_by`), funciones y métodos (`has_method`/`method_of`), interfaces, importaciones (`imports`/`imported_by`) y grafos de llamadas. La propagación del radio de impacto (Blast Radius) opera a través de fronteras políglotas enlazando módulos dependientes y suites de pruebas en un solo paso BFS. Validado por suite CTest 38/38 (`tests/test_code_graph_polyglot.c`) con 0 regresiones en las 53 pruebas previas de C, 2026-09-20.

## Harness de Evaluación SWE-bench Lite (Fase 9)

- El harness de verificación quirúrgica SWE-bench Lite (`src/swe_bench_harness.c`, `include/swe_bench_harness.h`) evalúa la fase de verificación previa de parches y análisis heurístico de impacto sobre fixtures de repositorios (Django, Flask, Sympy, Scikit-learn y Pytest). Coordina instanciación en espacio de trabajo, grafo de impacto políglota, pre-verificación quirúrgica (`PatchVerifyPlan`), aplicación y rollback local (`PatchApplyAtomic`, `PatchRollback`) y telemetría de proceso en el harness. No son transacciones de sistema de archivos ni resultados upstream de SWE-bench. Validado por suite CTest 36/36 (`tests/test_swe_bench_harness.c`), 2026-09-20.

## Indexador de Repositorios, CLI symbols-agent y Dogfooding OpenCode (Fase 10)

- El indexador recursivo nativo en C11 (`CodeGraphIngestDirectory`) recorre árboles de repositorios y extrae símbolos mediante heurísticas, filtrando carpetas de compilación, control de versiones y entornos virtuales (`.git`, `build*`, `node_modules`, `venv`, etc.). `symbols_server` auto-detecta rutas de repositorios y monta en RAM el grafo de símbolos y llamadas del proyecto seleccionado cuando la ruta es compatible. El CLI `symbols-agent` (`src/agent_cli_main.c`) permite inspección de impacto (`--blast-radius`), diagnóstico abductivo (`--diagnose`) y comandos de tarea acotados en terminal local. Validado por suite CTest (`test_code_graph_indexer.c`) 100% PASS y sesión de dogfooding real HTTP multi-turno con OpenCode (`tools/test_opencode_live_dogfood.py`), 2026-09-20.

## Motor de Ejecución de Shell Multiplataforma y Bucle de Autocuración (Fase 11)

- El motor de ejecución de subprocess nativo en C11 (`src/agent_shell.c`, `include/agent_shell.h`) unifica la ejecución de shells nativos en Windows (`cmd.exe`, `powershell.exe`), Linux (`/bin/bash`, `/bin/sh`) y macOS (`/bin/zsh`, `/bin/sh`) sin dependencias externas. Incorpora captura de doble flujo (`stdout` y `stderr` independientes de hasta 64 KB), loop de drenaje no-bloqueante anti-deadlock de pipes, protección por timeout de precisión milimétrica (código de salida 124 y terminación forzosa del proceso hijo), y telemetría de latencia de reloj. Integrado de extremo a extremo en `AgentRunnerSolveTask` (`src/agent_runner.c`): ante fallos en comandos de construcción o pruebas (`exit_code != 0`), el flujo de error alimenta automáticamente al motor abductivo `DiagnosticParseOutput`, dispara replanificación dinámica STRIPS (`AgentPlannerReplanOnError`), y dispone de rollback local (`PatchRollback`), sin garantía transaccional entre procesos. Validado por suite CTest 57/57 (`tests/test_agent_shell.c`) con 0 regresiones en suites agénticas, 2026-09-20.

## Corpus de Programación en C y Síntesis Acotada con Autocuración GCC (Fase 12)
...
- El sistema incorpora un doble corpus especializado para programación en C11:
  ...
  2. Modelo de grafo de la biblioteca estándar de C (`data/c_lang/c_std_lib.h`) indexado en el Grafo de Conocimiento del Código (`CodeGraphIngestFile`), permitiendo consultas de firmas y estructuras de libc (`C_FILE`, `malloc`, `snprintf`, `memcpy`, etc.), sin garantía de tiempo constante.
  3. Bucle cerrado de síntesis y autocuración de código C: síntesis de los módulos C cubiertos por el harness y compilación con GCC bajo `-Wall -Wextra -Werror` vía `AgentShellExec`, con ejecución determinista de binarios verificados y autodiagnóstico abductivo (`DiagnosticParseOutput` + parche local) ante errores de compilador. Validado por suite CTest 43/43 (`tests/test_c_synthesis.c`) con 12/12 suites agénticas en verde, 2026-09-20.

## Higiene y Corrección de Suites CTest (Fase 13)

- Corrección integral de pruebas unitarias y de integración en CTest:
  1. `test_composite`: corrección en `src/chat.c` en el interceptor de preguntas estructurales (`has_frozen_kw`), integrando términos de cónyuge (`wife`, `husband`, `esposa`, `esposo`), hermanos (`brother`, `sister`, `hermano`, `hermana`) y soberanos (`king`, `queen`, `rey`, `reina`), evitando que la relación fuese clasificada erróneamente como entidad; y resolución de anáfora con pronombres no resueltos (`he`, `she`, `it`) retornando UNKNOWN en los fixtures probados y span-echo (`No tengo constancia suficiente...`). Test 9/9 PASS.
...
- Implementación completa, determinista y de cero dependencias en C11 de los 4 pilares cognitivos de `ROADMAP.md`:
  1. **Pilar 1: Computación Hiperdimensional y Arquitectura Simbólica Vectorial (VSA / HDC)** (`src/vsa.c`, `include/vsa.h`): Vectores Kanerva de 256 bits (32 bytes/vector en los ABI probados), operaciones de enlace/desenlace XOR y limpieza asociativa; las pruebas incluyen prototipos con ruido de 15 bits y consultas de roles. Validado por `test_vsa` (8/8 PASS).
  2. **Pilar 2: Realización de Superficie Dinámica y Gramática Categorial Combinatoria (CCG)** (`src/ccg_realizer.c`, `include/ccg_realizer.h`): Cálculo formal de categorías CCG ($S, NP, N, PP, ADJ$ y combinadores $>$, $<$, $>B$, $<B$, $\&_\Phi$), tablas declarativas de concordancia morfosintáctica y preposiciones (`HARDCODING=0`) en EN/ES/FR, realización en prosa de 5 topologías de subgrafo y reductor de carta (`CcgVerifyReduction`) probado sobre casos definidos. Validado por `test_ccg_realizer` (34/34 PASS).
  3. **Pilar 3: Ingesta de Sentido Común y Ontologías a Gran Escala en RAM** (`src/commonsense.c`, `include/commonsense.h`): Parser de flujo para datos de ConceptNet (TSV de 5 columnas con extracción URI; el snapshot local de Fase 27 usa 5.7.0), mapa declarativo de canonicalización ontológica (`HARDCODING=0`) para 17 relaciones ontológicas, estructura `RELATION` de 32 bytes en los ABI probados, sin incluir índices ni cadenas, y razonamiento sobre contención espacial transitiva, affordances funcionales y causalidad física. Validado por `test_commonsense` (54/54 PASS).
  4. **Pilar 4: Condicionamiento Pragmático, Perspectivas Epistémicas y Filtros de Persona** (`src/persona.c`, `include/persona.h`): Operador de proyección matemática $\Pi_{\text{style}} : \mathcal{G} \to \mathcal{G}_{\text{biased}}$ sobre el Meta-Grafo Reflexivo en los casos implementados. 6 perspectivas epistémicas (`neutral`, `architect`, `auditor`, `tutor`, `concise`, `socratic`), perfiles declarativos, umbrales epistémicos y abstención en EN/ES/FR. `PersonaVerifyNonInterference` comprueba no-interferencia factual solo en sus fixtures; no demuestra un teorema general ni cero alucinaciones fuera de ellos. Validado por `test_persona` (40/40 PASS).
...
- Implementación estrictamente declarativa y de cero dependencias del perfil pragmático `PERSONA_PIRATE_QUANTUM` en `src/persona.c` e `include/persona.h`:
  ...
  2. Realización causal física genérica: `PersonaRealizePhysicalConsequence` opera composicionalmente sobre los argumentos ontológicos (`subject`, `action`, `target`, `material`, `consequence`), con comprobaciones de no-interferencia factual en la suite de pruebas.
  ...
  4. Verificación histórica sobre fixtures: 56/56 pruebas en `test_persona.c`, y suite CTest 74/74 PASS (65 Passed, 9 Skipped, 0 Failed), 2026-09-20.
...
- Implementación del subsistema de memoria episódica continua en C11 (`src/episodic_memory.c`, `include/episodic_memory.h`):
  1. **Almacén TSV persistente** (`data/memory/episodic.tsv`): registro estructurado con tuplas `<subject>\t<relation>\t<object>\t<source>\t<timestamp>`, deduplicación mediante búsqueda lineal y escritura del TSV sin reemplazo atómico ni propagación completa de errores de guardado.
  ...
  3. **Comandos conversacionales y aprendizaje natural en vivo**:
     - Comandos interactivos: `/learn S P O` o `/aprende S P O`, `/memory` (inspección de memorias activas con origen y marcas de tiempo), y `/forget` o `/olvida` (borrado del almacén, sin prueba de purga de hechos ya inyectados en el grafo del proceso).
     ...
     - Consulta de las relaciones aprendidas en los patrones soportados: las relaciones aprendidas se auto-registran en el índice dinámico `ch->kws`, permitiendo responder consultas directas posteriores (ej. `¿quién es el maestro de Platón?` -> `Sócrates`) entre reinicios de sesión.
  4. Pruebas históricas: suite unitaria dedicada `tests/test_episodic_memory.c` (5/5 PASS), suite CTest global al 100% (66 Passed, 9 Skipped condicionales, 0 Failed de 75 tests) y prueba cruzada de persistencia multi-sesión validada por ejecución.
...
- Implementación en C11 nativo del formato binario y mapeo de memoria virtual (`mmap` / `MapViewOfFile`) para grafos de conocimiento de sentido común (`src/commonsense.c`, `include/commonsense.h`):
  1. **Especificación Binaria Ultra-Densa (M3.4)**: cabecera `CS_BIN_HEADER` de 40 bytes alineada a 8 bytes (`magic: 0x53594D43`, versión, conteo de símbolos y relaciones, longitud de tabla de cadenas, flags, checksum FNV-1a de 32 bits), descriptores de símbolos de 16 bytes (`offset`, `len`, `frequency`), arena de cadenas empaquetada con padding de 8 bytes, y tabla de tripletas con `RELATION` de 32 bytes en los ABI probados.
  2. **Ingesta por Buffer y Snapshot**: funciones `CommonsenseSaveBinary`, `CommonsenseLoadBinary` y `CommonsenseParseBinaryBuffer` para serializar y deserializar grafos, pre-dimensionando las tablas de hashing (`GraphCreate(cap * 2)`) para evitar reallocs y rehashes durante la carga.
  3. **Mapeo de Memoria Virtual Multiplataforma**: `CommonsenseLoadMmap` y `CommonsenseMmapClose` implementan mapeo de memoria sin copias de archivo sobre Windows (`CreateFileA`, `CreateFileMappingA`, `MapViewOfFile`, `UnmapViewOfFile`) y POSIX (`open`, `mmap`, `munmap`), permitiendo mapear el archivo sin copiar todos sus bytes al inicio, sin garantía de latencia universal.
  4. **Fail-Closed y Resistencia a Corrupción**: controles de checksum, tamaño y cabeceras que rechazan los casos de corrupción probados; no son una prueba para toda entrada posible.
  5. **Integración Conversacional en `chat.c`**: `ChatGetCommonsenseGraph` busca y carga prioritariamente `data/commonsense.bin` si existe en disco antes de recurrir a la ingesta seed, sin promesa de latencia fija de arranque.
  6. **Pruebas históricas y CTest**: suite `tests/test_commonsense.c` ampliada a 75/75 verificaciones unitarias (incluyendo serialización, deserialización idéntica, persistencia mmap, latencia y rechazo de corrupción), y suite global CTest 100% verde (66 Passed, 9 Skipped condicionales, 0 Failed de 75 tests), 2026-09-20.

## Copiloto Local OpenCode y Despliegue de Servidor (Fase 19)
...
- Integración y despliegue del motor `symbols-server` como copiloto local permanente en OpenCode y entornos de desarrollo:
  ...
  4. **Pruebas extremo a extremo (`tools/test_opencode_copilot_e2e.py`)**:
     ...
     - Validación de flujo STRIPS acotado en los casos de código de la suite (`POST /v1/chat/completions` con `tools` -> emisión de `tool_calls` adaptadas a herramientas declaradas).
     - Cierre y terminación limpia del proceso sin fugas o cuelgues observados en la prueba.
...
## Relaciones Genéricas Probadas, Robustez Agéntica y Creación de Archivos (Fase 20)
...
- Consolidación del motor conversacional y servidor agéntico OpenAI Tool Calling (`src/chat.c`, `src/server_proto.c`, `src/symbols_server.c`):
  1. **Relaciones Genéricas en Grafos Binarios (`GenericRelToConn`, `ChatLoadModel`)**: Eliminada la barrera de relaciones predefinidas. Las relaciones personalizadas probadas en modelos binarios (`CONTIENE`, `REQUIRES`, `INCLUYE`, `APLICA_A`, etc.) se normalizan de forma composicional sin hardcoding (`HARDCODING=0`), cargando los hechos de los fixtures del harness (`ChatFactCount > 0` en sus casos). Soporte de artículos en preguntas (`¿qué incluye el kit_a?`, `a que aplica el kit_b`).
  2. **Detección e Invocación Quirúrgica para Creación de Archivos (`IsFileCreationTask`)**: Reconocimiento de peticiones de creación de archivos (`crea un fichero test.txt`, `nuevo archivo config.json`, `create file foo.c`), formulando un plan de una llamada a `write` con el archivo objetivo (no una transacción de archivos), evitando la ejecución espuria de compilación y ctest sobre archivos nuevos.
  3. **Gestión de Cargas HTTP de Gran Tamaño (`SERVER_BODY_MAX` a 2 MB)**: Ampliado el búfer de peticiones HTTP a 2 MB en memoria estática compartida (`g_http_body`), corrigiendo en los casos probados el fallo `400 Bad Request: bad content length` en sesiones multi-turno de OpenCode donde los esquemas de herramientas y el historial acumulado superaban 64 KB.
...
- Resolución conversacional de cortesía, identidad de copiloto y prevención de secuestro léxico en `symbols-server` y el motor `chat` (`src/symbols_server.c`, `src/server_proto.c`, `src/chat.c`, `src/tool_config.c`, `data/agentic/self.tsv`):
  ...
  2. **Priorización de Saludo e Identidad en `ChatHandleToBuf`**:
     - Ejecución preferente de `SelfAnswer(line, out, size)` antes de la vía rápida de texto (`ChatTryTextLine`), resolviendo los disparadores de cortesía y presentación implementados.
  3. **Fallback Compilado y Tabla Declarativa Externa (`data/agentic/self.tsv`, `tool_config.c`)**:
     ...
     - Creación de `data/agentic/self.tsv` para configuración declarativa y compatibilidad en las pruebas históricas.
...
- Desacoplamiento de textos bíblicos como corpus predeterminado en `symbols-server`, `chat_main` y scripts de inicio (`scripts/run_symbols_server.bat`, `scripts/run_symbols_server.ps1`, `scripts/run_symbols_server.sh`, `src/symbols_server.c`, `src/chat_main.c`):
  1. **Corpus de Programación en C como Predeterminado**:
     ...
     - Los scripts de arranque (`run_symbols_server.bat`, `.ps1`, `.sh`) configuran por defecto `CORPORA=data/c_lang/c_corpus.txt`, indexando el estándar C11 (gestión de memoria dinámica, tipos, invariantes de seguridad de buffers, libc) y el grafo heurístico de símbolos del repositorio (radio de impacto aproximado).
     - La Biblia (`data/texts/bible.txt`) y otros textos históricos permanecen disponibles bajo demanda explícita (`--corpus`, argumento CLI, o `/load`), pero ya no se montan de forma predeterminada, para reducir las interferencias léxicas observadas en las pruebas.
  2. **Aislamiento de Sesiones Stateless y Wrap-Around de Respuestas**:
     - Aislamiento de estado en los casos HTTP cubiertos por las pruebas: ante consultas individuales (`nmsg <= 1`), `symbols-server` restablece limpiamente el foco (`focus_valid = 0`) y el historial de oraciones mostradas (`ntshown = 0`), evitando que un nuevo cliente herede el contexto o sesgo léxico de peticiones previas.
...
## Tolerancia a Errores Tipográficos y Sintagmas Nominales en Síntesis de Código (Fase 25)

- Generalización de consultas de síntesis de código (`src/server_proto.c`, `src/symbols_server.c`):
  1. **Detección Difusa / Distancia de Edición en Algoritmos**:
     - Algoritmo de distancia de edición acotada (`EditDistance(tok, ref) <= 2`) y búsqueda de subcadenas para algoritmos canónicos (`fibonacci`, `factorial`, `quicksort`, `mergesort`).
     - Erratas tipográficas habituales como `funcion fibinacci en C`, `fibonaci`, `facturial`, etc., se reconocen de forma determinista como síntesis de código.
  2. **Clasificación de Sintagmas Nominales sin Verbo**:
     - Consultas formadas por sustantivo de función/algoritmo/código (`funcion`, `algoritmo`, `codigo`, `programa`, `metodo`, `ejemplo`) más especificación de lenguaje (`en c`, `en c11`, `in c`, `en python`, `en js`, etc.) se despachan directamente como síntesis de código sin requerir un verbo imperativo (`escribe`, `haz`).
     - Se previene la generación de planes STRIPS con herramientas de mutación de archivos y comandos de prueba (`CMakeLists.txt`, `cmake --build`, `ctest`) ante consultas informativas de programación.
  3. **Verificación**:
...
- Consolidación del conocimiento técnico y generación determinista de estructuras de datos canónicas (`data/c_lang/c_corpus.txt`, `data/c_lang/c_std_lib.h`, `src/server_proto.c`, `src/symbols_server.c`):
  ...
  2. **Modelo léxico de Biblioteca Estándar C11 en RAM**:
     - Declaración de tipos dinámicos canónicos (`C_NODE`, `C_VECTOR`) para el grafo heurístico; no constituyen una inspección AST de libclang.
...
## Snapshot local de sentido común (Fase 27)
...
## Avances acotados de septiembre de 2026 (commits posteriores a Fase 27)

- `a2ddbfec`: pregunta CLI opt-in `--ask-missing-goal` solo para la tarea tipada `stdout-goal-missing` y un C sin entradas; la pregunta no edita ni infiere el objetivo del lenguaje natural. El primer clasificador natural se descartó por preguntas inseguras en el holdout.
- `1f07a11`: continuación CLI con respuesta stdout aportada por el usuario, clave de workspace y búsqueda de un único edit seguro en el primer nivel que pasa compilación/ejecución. La salida se compara como una línea normalizada, no byte a byte. La validación prueba que el objetivo declarado es alcanzable bajo ese contrato, no que el objetivo sea semánticamente correcto; no se registra episodio durable. Véase `docs/abstain-and-ask.md`.
- `650741f`: inspector libclang opt-in de solo lectura para una TU C y una variante explícita de `compile_commands.json`; puede abstenerse con `unknown`, no edita ni alimenta el grafo heurístico ni el operador de parches. No cubre C++, referencias de todo el proyecto ni edición AST.
- `2f1ee43` y `80bdf08`: job Windows nativo con Ninja, Python 3.11 y `libclang==18.1.1`; corpus opcional pasó en el SHA `2f1ee43`, según `docs/ast-inspect.md`. No supone que toda la matriz esté verde en un SHA posterior.
- `a04ab07`: el límite de 50 MB del harness permanece para builds normales y se omite bajo ASan para evitar un fallo de umbral inducido por el sanitizer; no cambia el contrato de memoria de producción.

Estos son hitos concretos, no pruebas de ingeniería autónoma general ni de ausencia de alucinaciones sobre entradas arbitrarias.
