"""Keep-chat13 — ≥40-turn ONE continuous stateful stress mixing ALL prior edges.

Edges in one state: greet · warmth · impatient · incomplete · typo · cuéntame ·
unit short / en cristiano · Fib · 2F lie · Pell · da igual · espera · Ohm ·
robot · UNKNOWN · bit_circuit (if atoms exist; else UNKNOWN — never invent).
...
def build_script() -> list[dict]:
    """ONE continuous ≥40-turn stress mixing ALL prior edges in one state."""
    items: list[dict] = []
    bit_atoms = _bit_circuit_atoms_exist()

    def add(q, expect, **kw):
        items.append({"q": q, "expect": expect, **kw})

    # --- Spine: greet → warmth → tell → incomplete Fib → 2F lie ---
    ...
    add("espera", "banter", forbid_robotic=True, must_ack=True, spine=True)
    add(
        "unidad",
        "known",
        forbid_robotic=True,
    ...
    add(
        ...
        must_any=["Master", "programa", "robot"],
        edge="robot",
        spine=True,
    )
    # bit_circuit: cite atoms if present; else UNKNOWN — never invent
    if bit_atoms:
        add(
            "bit_circuit",
            "known",
    ...
    add("alma", "unknown", forbid_robotic=True, soft_mystery=True, edge="soft_mystery")
    add(
        "kepler",
        "known",
        forbid_robotic=True,
    ...
    add(
        ...
        "known",
        forbid_robotic=True,
        must_any=["F(n)=", "Fibonacci", "suma", "anteriores", "misma"],
        edge="incomplete",
    )
    ...
    add(
        ...
        "prove",
        forbid_robotic=True,
        must_any=["F(n)=", "Fibonacci", "sin inventar", "No invento", "Firmado"],
        edge="impatient",
    )
    ...
    # second bit_circuit surface (circuito de bits) — same atom rule
    if bit_atoms:
        add(
            "circuito de bits",
            "known",
...
def run_grill(round_name: str = "keep-chat13") -> dict:
    kb = _load_theory(THEORY)
    items = build_script()
    results = []
    counts = {
    ...
    (RUNS / "keep-chat13.json").write_text(
...
def write_md(payload: dict) -> None:
    ...
    lines.append("")
    (RUNS / "keep-chat13.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
