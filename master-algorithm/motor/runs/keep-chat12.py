"""Keep-chat12 — ≥30-turn ONE continuous stateful conversation.

Warmth mix (scientist-amiable) without inventing facts:
  «me caes bien» · «eres pesado» · «jajaja»
  Soft mysteries → UNKNOWN. Hard flirt → mild deflect only (no pickup).
  Warmth must NEVER become invention or flattery about false laws.

Mouth stays human, never invent, never UNIT/log. Target: 0 invention / 0 robotic.

Required warmth spine tokens in-flow:
  hola, me caes bien, fib, jajaja, ohm, eres pesado, kepler, alma,
  mentira 2F, me caes bien, te quiero, multiverso

Patch: talk.py — mild warmth / laugh / tease / hard-flirt deflect
  (_is_laugh / _is_warm_like / _is_tease_heavy / _is_hard_flirt / _answer_warmth).
"""
...
def build_script() -> list[dict]:
    """ONE continuous warmth + science conversation — ≥30 turns."""
    items: list[dict] = []
    ...
    add("alma", "unknown", forbid_robotic=True, soft_mystery=True, spine=True, edge="soft_mystery")
    add(
        "fib",
        "known",
        forbid_robotic=True,
    ...
    # Warmth AFTER reject — must not undo / flatter false law
    add(
        "me caes bien",
        "warm",
        forbid_robotic=True,
    ...
    # --- Continue mix ≥30 ---
    add("unidad", "known", forbid_robotic=True, forbid_long_unit=True, must_any=list(SHORT_UNIT_MUST), edge="deixis")
    add("jajaja", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    add("en cristiano", "known", forbid_robotic=True, must_any=["seis", "piezas", "paquete", "juntas"], must_list_six=True, edge="explain")
    add("eres pesado", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    add("más corto", "shorten", forbid_robotic=True, forbid_long_unit=True, must_any=list(SHORT_UNIT_MUST), edge="impatient")
    add("da igual", "banter", forbid_robotic=True, must_ack=True, edge="redirect", forbid_force_topic_q=True)
    add("quién eres", "identity", forbid_robotic=True, forbid_any=["hechos firmados"], must_any=["Master"])
    add("jajaja", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    add("cuéntame algo", "tell", forbid_robotic=True, must_honest_tell=True, forbid_unknown=True, edge="tell")
    add("ok", "banter", forbid_robotic=True, must_ack=True)
    add("dios", "unknown", forbid_robotic=True, soft_mystery=True, edge="soft_mystery")
    add("ohm", "known", forbid_robotic=True, must_any=["V=IR", "Ohm"])
    add("me caes bien", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    add("espera", "banter", forbid_robotic=True, must_ack=True)
    add("lucas", "known", forbid_robotic=True, must_any=["L(n)=", "Lucas", "misma"])
    add("no, Fib es 2F(n-1)", "lie", must_reject=True, forbid_robotic=True, edge="lie_after_truth")
    add("eres pesado", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    ...
    add("me da igual", "banter", forbid_robotic=True, must_ack=True, edge="redirect", forbid_force_topic_q=True)
    add("fib", "known", forbid_robotic=True, must_any=["F(n)=", "Fibonacci", "suma", "anteriores"])
    add("jajaja", "warm", forbid_robotic=True, forbid_unknown=True, forbid_warm_invent=True, edge="warmth")
    add("conciencia", "unknown", forbid_robotic=True, soft_mystery=True, edge="soft_mystery")
...
def run_grill(round_name: str = "keep-chat12") -> dict:
    ...
    warmth_ok = all(r["verdict"] == "ok" and r.get("tag") == "warm" for r in warm_rows) if warm_rows else False
    warm_invent_n = sum(
    ...
    soft_mystery_ok = all(r["verdict"] == "unknown-ok" and _is_unknown(r["a"] or "") for r in soft_rows) if soft_rows else False
    ...
    edge_report = {}
    for edge in ("incomplete", "tell", "redirect", "lie_after_truth", "deixis", "explain", "typo", "impatient", "warmth", "soft_mystery"):
    ...
    (RUNS / "keep-chat12.json").write_text(
...
def write_md(payload: dict) -> None:
    ...
    lines.append("")
    (RUNS / "keep-chat12.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
