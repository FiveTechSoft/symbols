"""Keep-chat6 — ≥30-turn free dialogue after Fib/Ohm closer soften.

Mix: Fib, Ohm topic switch, espera, mejor otra cosa, contradiction 2F,
unit+en cristiano, ok/sí/mm, UNKNOWN words, robot ask.

Fail if «Eso cuadra; punto» or «y cuadra.» or UNIT{ or fact-count on greet.
Score axes: invention-fail, robotic-fail. Target 0/0. Patch only if fail.
"""
...
# Hard ban after Mouth softener — old Fib/Ohm stock closers
STOCK_FAIL_PHRASES = (
    "Eso cuadra; punto",
    "eso cuadra; punto",
    "y cuadra.",
...
# Soft leftover stock (report, not auto-fail unless flagged)
STOCK_WATCH = (
    "Nada más pretendo",
    "nada más pretendo",
    "Firmado; sin adorno",
...
def _has_stock_fail(text: str) -> str | None:
    ...
    for p in STOCK_FAIL_PHRASES:
        if p in al:
            return p
    # also catch «…, y cuadra.» trailing stock
...
def build_script() -> list[dict]:
    ...
    # D: lucas → espera → y pell → para → dale → entiendo
    ...
    # I: unit → en cristiano → bio UNKNOWN → gracias
    add(
        "unit_protocell_levers",
        "known",
        forbid_robotic=True,
...
def grade(item: dict, a: str, tag: str | None = None, st: dict | None = None) -> str:
    ...
    # Hard fail: banned stock closers / UNIT{ / fact-count on greet
    ...
    if expect == "identity":
        ...
        if item.get("must_any"):
            # robot ask: accept programa/Master even if "robot" only in question
            must = item["must_any"]
            if not any(m.lower() in al or m in (a or "") for m in must):
                # soften: for robot ask, "programa" OR "Master" is enough with identity tag
...
def run_grill(round_name: str = "keep-chat6") -> dict:
    ...
    (RUNS / "keep-chat6.json").write_text(
...
def write_md(payload: dict) -> None:
    ...
    if not stocks:
        lines.append("- (none)")
    for s in stocks:
        lines.append(f"- `{s.get('phrase')}` on `{s.get('q')}` → {(s.get('a') or '')[:120]}")
    lines.append("")
