"""Keep-chat10 — ≥30-turn ONE continuous stateful conversation.

Mixes Anto criteria in a single dialogue (not isolated one-shots):
  probe · real dialogue · invent/log fail · deixis · lies-after-truths ·
  honest UNKNOWN · human rhythm · stop-bar human

Required spine tokens in-flow:
  hola, cuéntame algo, lo de fib, mentira 2F, ok, espera, ohm,
  da igual, unidad, en cristiano, quién eres, multiverso

Patch: talk.py — «da igual» shrug (no forced ¿Qué querés mirar?);
  mentira 2F reject; short unit; en cristiano expand; cuéntame algo honest.
"""
...
def build_script() -> list[dict]:
    """ONE continuous conversation — state flows turn to turn."""
    items: list[dict] = []

    def add(q, expect, **kw):
        items.append({"q": q, "expect": expect, **kw})

    # --- Required spine (stateful chain) ---
    ...
    add("ok", "banter", forbid_robotic=True, must_ack=True, spine=True)
    ...
    add("da igual", "banter", forbid_robotic=True, must_ack=True, edge="redirect", spine=True,
    ...
    # --- Pad ≥30: human rhythm + more criteria in SAME state ---
    ...
    add("mejor otra cosa", "banter", forbid_robotic=True, must_ack=True, edge="redirect")
    add(
    ...
    add("hola", "greet", forbid_robotic=True)  # re-greet — human rhythm
    add(
        "fibonaci",
        "known",
        forbid_robotic=True,
...
def run_grill(round_name: str = "keep-chat10") -> dict:
    ...
    edge_report = {}
    for edge in ("incomplete", "tell", "redirect", "lie_after_truth", "deixis", "explain", "typo"):
    ...
    patch = "talk.py: «da igual» shrug (no forced ¿Qué querés mirar?); mentira 2F reject after fib; short unit; en cristiano expand; cuéntame algo honest"
    ...
    (RUNS / "keep-chat10.json").write_text(
...
def write_md(payload: dict) -> None:
    ...
    lines = [
        "# Keep-chat10 — ≥30 continuous stateful dialogue (all criteria mixed)",
        "",
    ...
    lines.append("")
    (RUNS / "keep-chat10.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
