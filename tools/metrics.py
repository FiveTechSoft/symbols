...
def render(m):
    ...
    if b and "tasks_total" in b:
        ...
        L.append(f"| Engineering task bank ({b['tasks_total']} tasks, {len(b['passed_by_category'])} categories) | "
                 ...
                 f"dev {b.get('passed_by_split', {}).get('dev', '?')}, retired held-out {b.get('passed_by_split', {}).get('heldout', '?')} (seen since fa230f4); blind batch counted separately by Mimo with --counts-only, see COORDINATION.md), "
