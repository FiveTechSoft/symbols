...
function Test-Query {
    ...
    # Use raw JSON string to avoid PowerShell escaping issues
...
# ============================================================
$basicTests = @(
    @{Name="Q1: Who created the heavens and earth?"; Query="Who created the heavens and earth?"; ExpectedContains="God|Lord|LORD"},
    ...
    @{Name="Q10: What happened on the seventh day?"; Query="What happened on the seventh day of creation?"; ExpectedContains="rest|rested"}
)
...
# ============================================================
$numericTests = @(
    @{Name="N1: Age of Methuselah"; Query="How old was Methuselah when he died?"; ExpectedContains="969"},
    @{Name="N2: Duration of flood"; Query="How long did it rain during the flood?"; ExpectedContains="40|forty"},
    ...
    @{Name="N5: Population count"; Query="How many Israelites left Egypt?"; ExpectedContains="600|thousand|multitude|number"}
)
...
$negationTests = @(
    ...
    @{Name="Neg2: Non-existent entity"; Query="Who is the president of Narnia?"},
    @{Name="Neg3: Impossible question"; Query="What color is the number 7?"},
    @{Name="Neg4: Fictional character"; Query="Is Harry Potter in the Bible?"},
...
$semanticTests = @(
    ...
    @{Name="S3: Different wording"; Query="What is the story about a big boat?"; ExpectedContains="Noah|ark|flood"},
...
$edgeTests = @(
    ...
    @{Name="E11: Code snippet attempt"; Query="print('hello'); return 42;"}
)
...
try {
    $body = '{`"model`":`"symbols`",`"messages`":[{`"role`":`"user`",`"content`":`"Hello`"}],`"max_tokens`":50}'
    $chatResp = Invoke-RestMethod -Uri "$SERVER/v1/chat/completions" -Method Post -Body $body -ContentType "application/json"
...
try {
    $q1 = '{`"messages`":[{`"role`":`"user`",`"content`":`"Who was David?`"}]}'
    $q2 = '{`"messages`":[{`"role`":`"user`",`"content`":`"Who was his father?`"}]}'
    
    $resp1 = Invoke-RestMethod -Uri "$SERVER/v1/chat/completions" -Method Post -Body $q1 -ContentType "application/json"
    $resp2 = Invoke-RestMethod -Uri "$SERVER/v1/chat/completions" -Method Post -Body $q2 -ContentType "application/json"
    
    if ($resp2.choices[0].message.content) {
        Write-Host "  PASS: Session handling works" -ForegroundColor Green
        $PASS++
    } else {
        Write-Host "  FAIL: Session handling failed" -ForegroundColor Red
        $FAIL++
    }
} catch {
