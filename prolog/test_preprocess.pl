...
test_preprocess :-
    Lines = [
        ...
        'So she was considering in her own mind as well as she could, for the hot day made her feel sleepy and stupid,',
    ...
    maplist([L]>>(
        preprocess_sentence(L, Simple),
        tokenize_pos(Simple, Tokens),
        ( parse_svo_clean(Tokens, SVO) -> true ; SVO = failed ),
        format('~w~n  preproc: ~w~n  tokens:  ~w~n  svo:     ~w~n~n', [L, Simple, Tokens, SVO])
    ), Lines).

:- test_preprocess, halt.
