...
swipl -s experiment41.pl -g experiment41 -t halt
...
- `experiment39.pl` — transferencia horizontal: concepto cA persistido
  (firma + skill + roles), wipe total de A (`facts=0`), dominio B con
...
- `experiment41.pl` — roles múltiples: `zorin` en cA (suministro) y
  cB (viaje) con la skill de cada rol y prueba independiente
  (identidad ≠ rol ≠ comportamiento); A-only/B-only y parciales →
  UNKNOWN.
- `experiment42.pl` + `natural_parse.pl` + `make_corpus_natural.py` +
  `corpus_natural/` — primer corpus abierto (656 frases, gold,
  held-out previo, queries, distractores); parser sin entidades,
  motor intacto.
...
- `experiment47.pl` — EXP47: ontología B contada en diálogo
...
- `experiment49.pl` — EXP49: hipótesis con confianza por soporte
  (`tours` aceptado por cadena, `allures` rechazado por inversión
  de roles con razón); el mapa solo registra lo aceptado.
- `experiment50.pl` — EXP50: 5 episodios sin reinicio (baseline,
  2 ontologías, reversal, no-olvido); tabla hechos/checks/ms por
  episodio; memoria monótona verificada.
