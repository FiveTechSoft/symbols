% experiment42.pl
% EXPERIMENT 42 - FIRST NATURAL CORPUS (abierto, no disenado)
% Runner sobre corpus_natural/corpus.txt con natural_parse.pl (cero
% lexicon de entidades). El motor conceptual NO se modifica: ingesta,
% descubrimiento por composicion (composition.pl), queries con prueba.
% El gold solo puntua extraccion; held-out se decidio ANTES de generar.
% Metricas: extraccion P/R + hallucination=0; reglas descubiertas;
% queries (retrieval/reasoned/unknown/distractors) + split core/tra.
:- consult('memory.pl').
:- consult('composition.pl').
:- consult('natural_parse.pl').
:- consult('corpus_natural/gold.pl').
:- consult('corpus_natural/expected.pl').
:- consult('corpus_natural/distractor_natural.pl').
...
% ---------- ingesta ----------
ingest_corpus :-
    open('corpus_natural/corpus.txt', read, S, [encoding(utf8)]),
    ingest_lines(S, 1),
    close(S),
    findall(1, parsed_line(_, _), Ps), length(Ps, NP),
    format('ingested lines with triplet: ~w~n', [NP]).
...
% ---------- extraccion: P/R + hallucination=0 ----------
extraction_report :-
    findall(N, gold_fact(N, _, _, _), GNs), length(GNs, NG),
    findall(N, (parsed_line(N, T), T \== none,
                gold_fact(N, S, R, O), T == (S, R, O)), TPs),
    length(TPs, TP),
    findall(N, parsed_line(N, T), PNs),
    findall(N, (member(N, PNs), parsed_line(N, T), T \== none), PPos),
    length(PPos, NPpos),
    ( NG > 0 -> P is TP / NG ; P = 0 ),
    ...
    % hallucination: tripleta parseada con entidad no presente en la frase
    findall(N-T, (parsed_line(N, T), T \== none,
                  \+ gold_fact(N, _, _, _),
                  \+ triple_in_sentence(N, T)), Halls),
    length(Halls, NH),
    ( NH =:= 0 ->
...
triple_in_sentence(N, (S, R, O)) :-
    gold_fact(N, S, R, O).

% ---------- conceptos (maquinaria local, como EXP33/36-41) ----------
...
% ---------- descubrimiento latente (sin decir que buscar) ----------
discover_latent :-
    discover_composition(reaches, 3),
    ( composed_rule(reaches, [visits, in], F) ->
        ( format('discovered: reaches :- [visits,in] F1=~4f~n', [F]),
          check(true, 'latent reaches :- [visits,in] discovered') )
    ; check(false, 'latent reaches :- [visits,in] discovered') ).
...
answer_reach(P, K, reasoned, Proof) :-
    composed_rule(reaches, [visits, in], _),
    memory_relation(P, visits, C, _, _),
