...
## EXP-META-UTILITY v1 (2026-09-17, chat.pl intacto)

**Cerrado (utilidad practica del hook GEN-META v3: A/B chat.pl CON hook vs chat_head.pl BASE sin hook, mismos loads bb_load(jonah)+consult(schema_kb.pl), mismo teach 6 lineas + 14 queries NUEVAS U1a..U8; checker 8/8 ALL PASS, idempotencia CONx2 MD5 F632521BF01E56E02325D713421EE63D identica).** Veredictos: DIFF = 6 filas exactas [U1a,U2a,U2b,U4a,U6a,U8] (atribucion exacta al hook: 4 fills declarados + 1 inversion simetrica U2b + 1 composicion U8); must_match byte-identico {U1b,U1c,U3a,U3b,U3c,U4b,U5a,U7} (hook invisible donde meta no aplica: inversions ord rechazadas U1b; graph_ask pre-existente ya respondia U1c/U3a/U3b/U3c/U4b con echo + proof en BASE; consumidor estricto U5a flowing no declarado; gate bb_content(ghost) U7). Mecanismo U8 verificado probe_he5: dialog_substitute (pre-existente dialog_ref.pl, compartido con BASE) resuelve he->massa en chat_line ANTES de chat_line_tokens (stack(subj)=[massa], tier_single 1 candidato), hook llena [what,is,massa,cong,to] -> answer([massb],[(massa,cong,massb)]); BASE sustituye igual pero sin fill cae a unknown = capacidad nueva COMPLEJA (composicion de capacidad pre-existente + hook), el hook solo NO dispara sobre pronombre crudo. Enmienda pre-pass documentada en battery (U3a meta->must_match; U8 must_match->meta; expected_diff_rows 6 filas reordenadas, count invariante). Hallazgo de alcance: fills cubren 4/6 filas meta-aplicables; U1c/U3a-family y grafos pre-existentes absorben el resto -> utilidad del hook = fills declarados + sim inversion + composicion pronominal, SIN regresion ni alucinacion (A8: toda linea de hecho CON/BASE = superficie de hecho ensenado). chat.pl SIN cambios (medicion-only). Artefactos: Temp\opencode\genmeta3\battery_metautility1.json (amendada, JSON valido), run_utility.pl, check_utility1.py (8/8), util_con1/2.txt util_base1.txt (UTF-16), probes he3/he4/he5. Ver mem-r30 en AgentBrain.

## EXP-COMPOSITION v1 (2026-09-17, chat.pl intacto)

**Cerrado (composicion multi-etapa: combinaciones de capacidades ya demostradas sobre vocabulario nuevo disjunto; A/B chat.pl CON hook 03eb2e0 vs chat_head.pl BASE e723ed9, mismos loads bb_load(jonah)+consult(schema_kb.pl), secuencia entrelazada 10 teach + 14 queries C1..C14 con orden load-bearing; checker 8/8 ALL PASS, idempotencia CONx2 MD5 3F13146A22CD0119A863C8E86278292C, doble-metodo DIFF verificado).** DIFF medido = 5 filas exactas {C1,C2,C7,C8,C13} (predichas 7; 2 predicciones fallidas con atribucion por probes): C1/C2 pronoun-tier+hook fill ord (he/she->vax unicos, fill mol, BASE unknown+GAP); C8 pronoun+graph_narrow+hook fill acting_on (he multi-candidato -> narrow por con keeps zud -> fill kref); C13 named re-teach (hook first-wins massb, zunt nunca devuelto); **C7 NUEVO: hook RESCATA pending-resume roto** (C6 clarify setea pending; resume 'krat' pierde is_question -> BASE bare unknown SIN GAP, CON re-entra como pregunta he-subj y llena via sym = capacidad emergente nueva del hook). Predicciones fallidas atribuidas por 4 probes: C4/C5 clarify pre-hook (it es both-tier, no obj-tier; tras C1 fill-note + T2 teach ambas tiers tienen >=2 candidatos -> dialog_graph_resolve clarify dispara ANTES del hook, identico en ambos; probe_comp10 replica bateria exacta; probe_comp9 estado limpio: it SI llena via sym en CON, BASE unknown; probe_comp12: inversion simetrica real via pronoun obj-tier VERDADERO 'him' -> CON fill plim, BASE unknown+GAP); C9 about-branch intercepta 'what is he after?' como pregunta about 'after' (chat.pl:1050 pre-existente, ambos engines) ANTES de dialog/pronoun. CONFLICT gate medido: 'CONFLICT held' (conflict.pl:72) dispara solo con >=2 OTROS sujetos clean_active misma rel (probe_comp1: dorp+krat N=2 disparo; bateria N=1 krat -> SILENTE en ambos engines). Hallazgo de mecanismo: hook alcanzable SOLO con candidato unico (tier-single he/she, obj-tier him/them, named) o estado limpio; both-tier it con >=2 -> clarify pre-hook; primer-wins sym bajo conflicto real (C13). Sin alucinacion (A7: toda linea de hecho = superficie ensenada o maquinaria clarify/about). chat.pl SIN cambios (medicion-only). Artefactos: Temp\opencode\genmeta3\battery_composition1.json (amendada con measured_* + prediction_audit + JSON valido), run_composition.pl (markers === ID ===), check_composition1.py (8/8, GAP counters normalizados #N), comp_con1/2.txt comp_base1.txt (UTF-16), probes comp9..comp12. Ver mem-r31 en AgentBrain.
