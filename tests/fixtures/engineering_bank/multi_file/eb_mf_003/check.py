...
if '#include "util.h"' not in _t:
    print('missing', '#include "util.h"', 'in', 'main.c')
    ok = False
_t = Path('util.c').read_text(encoding='utf-8', errors='replace') if Path('util.c').is_file() else ''
if '#include "util.h"' not in _t:
    print('missing', '#include "util.h"', 'in', 'util.c')
    ok = False
_t = Path('util.h').read_text(encoding='utf-8', errors='replace') if Path('util.h').is_file() else ''
if 'int add(int a, int b);' not in _t:
    print('missing', 'int add(int a, int b);', 'in', 'util.h')
    ok = False
