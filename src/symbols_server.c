...
static void ServerSynthesizeCode(const char *query, char *out, size_t out_sz)
{
    ...
    if (strstr(lower, "quicksort") != NULL || strstr(lower, "mergesort") != NULL ||
        strstr(lower, "ordenar") != NULL || strstr(lower, "sort") != NULL)
    {
        snprintf(out, out_sz,
            ...
            "}\n\n"
            "/**\n"
            " * Particion de Lomuto para Quicksort.\n"
            " */\n"
            ...
            " * Complejidad: O(n log n) promedio, O(1) memoria auxiliar.\n"
            " */\n"
            ...
            "int main(void)\n"
            "{\n"
            "    int nums[] = {64, 34, 25, 12, 22, 11, 90, 42};\n"
            "    int n = sizeof(nums) / sizeof(nums[0]);\n\n"
    ...
    if (strstr(lower, "lista") != NULL || strstr(lower, "linked list") != NULL ||
        strstr(lower, "nodo") != NULL || strstr(lower, "node") != NULL)
    {
        snprintf(out, out_sz,
    ...
    if (strstr(lower, "vector") != NULL || strstr(lower, "array dinamico") != NULL ||
        strstr(lower, "dynamic array") != NULL)
    {
        snprintf(out, out_sz,
            "Aqui tienes la implementacion de un array dinamico (vector redimensionable) en C (C11):\n\n"
            "```c\n"
            "#include <stdio.h>\n"
            "#include <stdlib.h>\n"
            "#include <stdbool.h>\n\n"
            ...
            "        int *new_items = (int *)realloc(vec->items, new_cap * sizeof(int));\n"
            "        if (new_items == NULL)\n"
            "            return false;\n"
            "        vec->items = new_items;\n"
            ...
            "        VectorPush(&vec, i * 10);\n"
            "    }\n"
            "    printf(\"Vector (size=%%zu, cap=%%zu): \", vec.size, vec.capacity);\n"
    ...
    if (strstr(lower, "leer archivo") != NULL || strstr(lower, "leer fichero") != NULL ||
        strstr(lower, "read file") != NULL || strstr(lower, "archivo") != NULL ||
        strstr(lower, "fichero") != NULL || strstr(lower, "file") != NULL)
    {
        snprintf(out, out_sz,
            ...
            "#include <string.h>\n\n"
            "/**\n"
            " * Lee un archivo de texto linea por linea de forma segura con fgets.\n"
            " */\n"
            ...
            "int main(void)\n"
            "{\n"
            "    ReadFileLineByLine(\"ejemplo.txt\");\n"
            "    return 0;\n"
            "}\n"
            "```");
        return;
    ...
    if (strstr(lower, "pila") != NULL || strstr(lower, "stack") != NULL ||
        strstr(lower, "cola") != NULL || strstr(lower, "queue") != NULL)
    {
        snprintf(out, out_sz,
    ...
    if (strstr(lower, "qsort") != NULL)
    {
        snprintf(out, out_sz,
            ...
            "#include <stdlib.h>\n\n"
            "/**\n"
            " * Funcion de comparacion para qsort (orden ascendente).\n"
            " */\n"
            ...
            "    printf(\"Valores ordenados con qsort: \");\n"
